...
export default function ReportesPage() {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Reporte_${format(new Date(), "dd-MM-yyyy")}.pdf`)
    } catch (error) {
            ...
            <div ref={reportRef} className="space-y-8 p-6 bg-[#0a0a0a] rounded-xl border border-white/5">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="h-72 bg-[#121212] p-4 rounded-xl border border-white/5">
                    <p className="text-sm font-medium mb-4 text-gray-400 text-center">Comparativa Mensual</p>
                    ...
                    <table className="w-full text-left text-sm">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {transactions.length === 0 ? (
                            <tr>
                                <td colSpan={4} className="p-8 text-center text-gray-500">No se encontraron movimientos en este rango de fechas.</td>
                            </tr>
                        ) : (
                            transactions.map((t) => (
                              <tr key={t.id} className="bg-[#121212] hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-3">
                                  <span className="px-2 py-0.5 rounded-full bg-white/5 text-[10px] text-gray-300 border border-white/10 uppercase">
                                    {t.categoria}
                                  </span>
                                </td>
