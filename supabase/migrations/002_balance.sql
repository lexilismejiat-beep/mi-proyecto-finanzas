...
-- trim(tipo): el campo tipo puede venir con espacios (bot de Telegram y
-- ediciones antiguas), igual que el resto del código que hace t.tipo?.trim().
-- security invoker: corre con los privilegios de quien llama, respeta la RLS.
-- set search_path = public: evita resolución de nombres ambigua/insegura.
...
as $$
  select
    ...
    + coalesce((
        select sum(
          case
            when trim(tipo) = 'Ingreso' then monto_cop
            when trim(tipo) = 'Egreso' then -monto_cop
            else 0
          end
        )
        from transacciones
